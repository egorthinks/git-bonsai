...
async function run() {
    ...
    if (process.env.GITHUB_OUTPUT) {
        fs.appendFileSync(process.env.GITHUB_OUTPUT, `style=${out.dna.style}\nspecies=${out.dna.species}\nsize-class=${out.dna.sizeClass}\n`);
    }
    ...
    if (process.env.GITHUB_STEP_SUMMARY && process.env.GITHUB_REPOSITORY) {
        ...
        const text = encodeURIComponent(`My GitHub history grew a ${out.dna.style} ${out.dna.species} bonsai 🌳 #gitbonsai\n` +
            `https://github.com/${repo}`);
        fs.appendFileSync(process.env.GITHUB_STEP_SUMMARY, `## 🌳 Your bonsai has been tended\n\n` +
